...
function fixName(name, isScope) {
  ...
  var match = name.match(scopedPackagePattern)[1] && name.match(scopedPackagePattern)[2];
  if (match) {
    const user = fixName(match[1], true) ? encodeURIComponent(match[1]) : null;
    const pkg = fixName(match[2]) ? encodeURIComponent(match[2]) : null;
    if (user && pkg) {
      return `@${user}/${pkg}`;
    }
  }
